-- EMERGENCY SECURITY FIX: Secure leads table with user isolation (Fixed approach)
...
VALUES (
  ...
  'system@specter-insights.com',
  crypt('system_placeholder_password', gen_salt('bf')),
  now(),
  now(),
  now()
) ON CONFLICT (id) DO NOTHING;

-- Step 2: Add user_id column to leads table if it doesn't exist
ALTER TABLE public.leads ADD COLUMN IF NOT EXISTS user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE;

-- Step 3: Update existing records to use system user
UPDATE public.leads 
...
-- Step 5: Drop existing insecure public policies
...
-- Step 6: Create secure user-specific RLS policies
...
-- Step 7: Secure competitors table
