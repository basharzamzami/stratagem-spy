...
router.get('/', async (req, res) => {
  try {
    ...
    res.json({ status: 'success', data: leads, meta: { total: leads.length } });
  } catch (error) {
...
router.get('/search', async (req, res) => {
  try {
    ...
    res.json({ status: 'success', data: filtered, meta: { total: filtered.length } });
  } catch (error) {
    res.status(500).json({ status: 'error', message: 'Failed to search leads' });
  }
});
...
router.post('/:id/enrich', async (req, res) => {
  try {
    ...
    res.json({ status: 'success', data: leads[index] });
  } catch (error) {
    res.status(500).json({ status: 'error', message: 'Failed to enrich lead' });
  }
});
