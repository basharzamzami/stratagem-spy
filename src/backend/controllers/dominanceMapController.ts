...
import type { DominanceData } from '../types/index.js';

export const getDominanceData = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Failed to fetch dominance data' });
  }
};

export const getDominanceSummary = async (req: Request, res: Response) => {
  try {
    const dominanceData = await readJsonFile<DominanceData>('dominanceMap.json');
